...
pub fn unpack_webpack_chunk_list(source: &ParsedSource) -> Option<Vec<(String, String)>> {
    ...
    for maybe_prop in &chunk_list.props {
        ...
        let key = kv.key.as_num().unwrap().raw.as_ref()?.to_string();
        let chunk_range: SourceRange = match &*kv.value {
            Expr::Fn(fun) => {
                // for cases like `<key>: function (<args>) {<body>}`, a function expr
                // without an identifier is invalid on its own. for now we just chop out
                // <body>, but this isn't great since it loses info about the args
                fun.function.body.as_ref().unwrap().range()
            }
            Expr::Arrow(arrow) => arrow.range(),
            _ => panic!("unknown chunk list value {:?}", &kv.value),
        };
